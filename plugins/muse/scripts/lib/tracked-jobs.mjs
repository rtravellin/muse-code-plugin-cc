...
export const SESSION_ID_ENV = "MUSE_CC_SESSION_ID";
...
export function createProgressReporter({ stderr = false, logFile = null, onEvent = null } = {}) {
  ...
  return (eventOrMessage) => {
    ...
    if (stderr && stderrMessage) {
      process.stderr.write(`[muse-cc] ${stderrMessage}\n`);
    }
...
export function resolveJobKillTargets(job = {}) {
  const targets = [];
  const seen = new Set();
  for (const value of [job.agentPid, job.bridgePid, job.pid]) {
    if (value == null || value === "") {
      continue;
    }
...
export async function runTrackedJob(job, runner, options = {}) {
  ...
  try {
    ...
    const claim = claimJobTerminal(job.workspaceRoot, job.id, completionStatus, {
      ...
      ...(execution.worktreePath ? { worktreePath: execution.worktreePath, worktreeBranch: execution.worktreeBranch ?? null } : {}),
      bridgePid,
      agentPid: null,
      pid: null,
